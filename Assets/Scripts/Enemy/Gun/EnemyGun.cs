...
public class EnemyGun : MonoBehaviour
{
    [HideInInspector] public Transform spawnBullet;
    [HideInInspector] public GameObject bulletPrefab;
    [HideInInspector] public float speedBullet;
    [HideInInspector] public float sizeBullet;
    [HideInInspector] public float damageBullet;
    [HideInInspector] public float radiusBullet;
    [HideInInspector] public float intervalBullet;
    [HideInInspector] public float precisionBullet;
    ...
    private void Shoot(Transform target)
    {
        ...
        Vector3 dir = (target.position - spawnBullet.position).normalized;

        // Cálculo de Precisão
        float errorAmount = 45f * (1f - (precisionBullet / 100f));
        float errorX = Random.Range(-errorAmount, errorAmount);
        float errorY = Random.Range(-errorAmount, errorAmount);
        Vector3 finalDirection = Quaternion.Euler(errorX, errorY, 0) * dir;

        GameObject bulletObj = Instantiate(bulletPrefab, spawnBullet.position, Quaternion.LookRotation(finalDirection));
        bulletObj.transform.localScale = Vector3.one * sizeBullet;

        // Tenta pegar o script da bala (se ele existir) e passar os dados
        EnemyBullet bulletScript = bulletObj.GetComponent<EnemyBullet>();
        if (bulletScript != null) bulletScript.Setup(damageBullet, speedBullet);

        Rigidbody rb = bulletObj.GetComponent<Rigidbody>();
        if (rb != null) rb.velocity = finalDirection * speedBullet;
    }
